...
function questionSubquestionLabels(question) {
  const current = question || {};
  const explicit = current.subquestionLabels || current.subquestion_labels || current.subquestions || current.sub_questions;
  if (Array.isArray(explicit) && explicit.length) {
    return explicit.map((item, index) => String(typeof item === 'object' ? item.label || item.title || index + 1 : item).trim() || String(index + 1)).slice(0, 50);
  }
  const source = [current.prompt, current.body].filter(Boolean).join('\n');
  const labels = [];
  const pattern = /^[ \t]*(?:\(?([0-9]{1,2}|[A-Z가-힣])\)?[.)]|([①②③④⑤⑥⑦⑧⑨⑩]))[ \t]+/gm;
  let match;
  while ((match = pattern.exec(source))) labels.push(match[1] || match[2]);
  if (labels.length >= 2) return [...new Set(labels)].slice(0, 50);
  if (current.subquestionMode && Array.isArray(current.subquestionLabels) && current.subquestionLabels.length) return current.subquestionLabels;
  return current.subquestionMode ? ['1', '2'] : [];
}

function ensureQuestionSubquestionAnswers(question) {
  if (!question || !question.subquestionMode) return [];
  const labels = questionSubquestionLabels(question);
  if (!labels.length) return [];
  let answers = Array.isArray(question.subquestionAnswers) ? question.subquestionAnswers.map((item) => String(item || '')) : [];
  if (!answers.length && String(question.userAnswer || '').trim()) {
    const stored = String(question.userAnswer).split(QUESTION_SUBANSWER_SEPARATOR);
    answers = stored.length > 1 ? stored : [stored[0]];
  }
  while (answers.length < labels.length) answers.push('');
  answers = answers.slice(0, labels.length);
  const defaultFormat = question.codeEditorEnabled ? questionCodeLanguage(question) || 'text' : 'text';
  let formats = Array.isArray(question.subquestionFormats) ? question.subquestionFormats.map((item) => normalizeQuestionCodeLanguage(item) || defaultFormat) : [];
  while (formats.length < labels.length) formats.push(defaultFormat);
  formats = formats.slice(0, labels.length);
  question.subquestionFormats = formats;
  question.subquestionLabels = labels;
  question.subquestionAnswers = answers;
  question.userAnswer = answers.map((answer, index) => `[소문제 ${labels[index]}]\n${answer}`).join(QUESTION_SUBANSWER_SEPARATOR);
  return answers;
}

function questionSubquestionFormat(question, index) {
  ensureQuestionSubquestionAnswers(question);
  const stored = question?.subquestionFormats?.[index];
  return normalizeQuestionCodeLanguage(stored) || (question?.codeEditorEnabled ? questionCodeLanguage(question) || 'text' : 'text');
}

function questionSubquestionFormatOptions(selected) {
...
function updateQuestionSubquestionAnswer(question, index, value) {
  const answers = ensureQuestionSubquestionAnswers(question);
  if (!answers.length || !Number.isInteger(index) || index < 0 || index >= answers.length) return;
  answers[index] = String(value || '');
  question.subquestionAnswers = answers;
  question.userAnswer = answers.map((answer, answerIndex) => `[소문제 ${question.subquestionLabels[answerIndex]}]\n${answer}`).join(QUESTION_SUBANSWER_SEPARATOR);
}
...
function linkedQuestionKeywordCard(question = null) {
  const current = question && typeof question === 'object'
    ? hydrateQuestionState({...question})
    : hydrateQuestionState(currentQuestion());
  const linkedCardId = String(current?.card_id || '').trim();
  if (!linkedCardId) return null;
  return state.cards.find((card) => String(card?.id || '').trim() === linkedCardId)
    || {id: linkedCardId, term: String(current?.term || linkedCardId).trim() || linkedCardId};
}

function questionKeywordTargetCard(keyword, {question = null, reviewCard = null} = {}) {
  const matchedCard = findCardByKeyword(keyword);
  if (matchedCard) return matchedCard;
  const linkedCard = linkedQuestionKeywordCard(question);
  if (linkedCard) return linkedCard;
  if (reviewCard && typeof reviewCard === 'object' && String(reviewCard.id || '').trim()) return reviewCard;
  return null;
}

function renderQuestionKeywordLinks(keywords, {interactive = false, question = null, reviewCard = null} = {}) {
  const items = normalizeQuestionKeywords(keywords);
  if (!items.length) return '';
  return items.map((keyword) => {
    const text = escapeHtml(keyword);
    const targetCard = interactive ? questionKeywordTargetCard(keyword, {question, reviewCard}) : null;
    if (interactive && targetCard) {
      return `<button class="question-keyword-link" type="button" data-question-keyword="${text}" data-question-card-id="${escapeHtml(targetCard.id || '')}">${text}</button>`;
    }
...
function handleOpenModalKeydown(event) {
  ...
  if (event.key === 'Escape') {
    ...
    else if (dialog.id === 'questionBankEditDialog') closeQuestionBankEditDialog();
...
function revealOrFinishQuestion() {
  if (questionRevealLocked()) return finishQuestionSession();
  return revealQuestionAnswer();
}
...
function questionBankEditDialog() {
  return $('questionBankEditDialog');
}

function setQuestionBankEditError(message = '') {
  const errorEl = $('questionBankEditError');
  if (errorEl) errorEl.textContent = String(message || '').trim();
}

function setQuestionBankEditSaving(saving = false) {
  const disabled = Boolean(saving);
  questionBankEditDialog()?.querySelectorAll('input, textarea, select, button').forEach((element) => {
    element.disabled = disabled;
  });
  if ($('questionBankEditSaveBtn')) $('questionBankEditSaveBtn').textContent = disabled ? '저장 중…' : '저장';
}

function normalizeQuestionBankEditorLines(rawText) {
...
function parseOptionalQuestionBankEditorNumber(rawValue, {label = '숫자', min = 0, max = Number.MAX_SAFE_INTEGER} = {}) {
  const text = String(rawValue ?? '').trim();
  if (!text) return null;
  const value = Number.parseInt(text, 10);
  if (!Number.isInteger(value) || value < min || value > max) {
    throw new Error(`${label}는 ${min} 이상 ${max} 이하의 숫자로 입력하세요.`);
  }
  return value;
}

function fillQuestionBankEditDialog(question = currentQuestion()) {
  const current = hydrateQuestionState(question);
  if (!current?.questionBankId) return;
  const summary = $('questionBankEditSummary');
  if (summary) {
    const parts = [
      `문항 ID ${current.questionBankId}`,
      current.card_id ? `연결 카드 ${current.card_id}` : '단독 문항',
      questionSessionModeLabel(current.sessionMode || 'practice'),
    ].filter(Boolean);
    summary.textContent = parts.join(' · ');
  }
  if ($('questionBankEditTypeInput')) $('questionBankEditTypeInput').value = current.type || 'subjective';
  if ($('questionBankEditSessionModeInput')) $('questionBankEditSessionModeInput').value = normalizeQuestionSessionMode(current.sessionMode || 'practice');
  if ($('questionBankEditPromptInput')) $('questionBankEditPromptInput').value = current.prompt || '';
  if ($('questionBankEditBodyInput')) $('questionBankEditBodyInput').value = current.body || '';
  if ($('questionBankEditAnswerInput')) $('questionBankEditAnswerInput').value = current.answer || '';
  if ($('questionBankEditExplanationInput')) $('questionBankEditExplanationInput').value = current.explanation || '';
  if ($('questionBankEditRubricInput')) $('questionBankEditRubricInput').value = Array.isArray(current.rubric) ? current.rubric.join('\n') : '';
  if ($('questionBankEditChoicesInput')) $('questionBankEditChoicesInput').value = Array.isArray(current.choices) ? current.choices.join('\n') : '';
  if ($('questionBankEditAnswerIndexInput')) $('questionBankEditAnswerIndexInput').value = Number.isInteger(current.answer_index) ? String(current.answer_index + 1) : '';
  if ($('questionBankEditTopicInput')) $('questionBankEditTopicInput').value = current.topic || '';
  if ($('questionBankEditFieldNameInput')) $('questionBankEditFieldNameInput').value = current.fieldName || '';
  if ($('questionBankEditCategoryInput')) $('questionBankEditCategoryInput').value = current.category || '';
  if ($('questionBankEditDifficultyInput')) $('questionBankEditDifficultyInput').value = current.difficulty || '';
  if ($('questionBankEditIssuerInput')) $('questionBankEditIssuerInput').value = current.issuer || '';
  if ($('questionBankEditSourceLocationInput')) $('questionBankEditSourceLocationInput').value = current.sourceLocation || '';
  if ($('questionBankEditSectionInput')) $('questionBankEditSectionInput').value = current.section || '';
  if ($('questionBankEditPointsInput')) $('questionBankEditPointsInput').value = Number.isInteger(current.points) ? String(current.points) : '';
  if ($('questionBankEditExpectedTimeSecondsInput')) $('questionBankEditExpectedTimeSecondsInput').value = Number.isInteger(current.expectedTimeSeconds) ? String(current.expectedTimeSeconds) : '';
  if ($('questionBankEditAnswerGuideInput')) $('questionBankEditAnswerGuideInput').value = current.answerGuide || '';
  setQuestionBankEditSaving(false);
}

function questionBankEditPayload(question = currentQuestion()) {
  const current = hydrateQuestionState(question);
  const payload = current ? questionBankEntryPayload(current) : null;
  if (!current?.questionBankId || !payload) return null;
  payload.question_type = $('questionBankEditTypeInput')?.value || current.type || 'subjective';
  payload.session_mode = normalizeQuestionSessionMode($('questionBankEditSessionModeInput')?.value || current.sessionMode || 'practice');
  payload.prompt = $('questionBankEditPromptInput')?.value || '';
  payload.body = $('questionBankEditBodyInput')?.value || '';
  payload.answer = $('questionBankEditAnswerInput')?.value || '';
  payload.explanation = $('questionBankEditExplanationInput')?.value || '';
  payload.rubric = normalizeQuestionBankEditorLines($('questionBankEditRubricInput')?.value || '');
  payload.choices = payload.question_type === 'multiple_choice'
    ? normalizeQuestionBankEditorLines($('questionBankEditChoicesInput')?.value || '')
    : [];
  payload.answer_index = payload.question_type === 'multiple_choice'
    ? parseOptionalQuestionBankEditorNumber($('questionBankEditAnswerIndexInput')?.value || '', {label: '정답 번호', min: 1, max: 101})
    : null;
  if (Number.isInteger(payload.answer_index)) payload.answer_index -= 1;
  payload.topic = $('questionBankEditTopicInput')?.value || '';
  payload.field_name = $('questionBankEditFieldNameInput')?.value || '';
  payload.category = $('questionBankEditCategoryInput')?.value || '';
  payload.difficulty = $('questionBankEditDifficultyInput')?.value || '';
  payload.issuer = $('questionBankEditIssuerInput')?.value || '';
  payload.source_location = $('questionBankEditSourceLocationInput')?.value || '';
  payload.section = $('questionBankEditSectionInput')?.value || '';
  payload.points = parseOptionalQuestionBankEditorNumber($('questionBankEditPointsInput')?.value || '', {label: '배점', min: 0, max: 1000});
  payload.expected_time_seconds = parseOptionalQuestionBankEditorNumber($('questionBankEditExpectedTimeSecondsInput')?.value || '', {label: '권장 시간', min: 0, max: 86400});
  payload.answer_guide = $('questionBankEditAnswerGuideInput')?.value || '';
  return payload;
}

function openQuestionBankEditDialog() {
  const question = hydrateQuestionState(currentQuestion());
  if (!question?.questionBankId || state.questionLoading || state.questionSaving || state.markSaving || state.questionAnswerRefineLoading || state.questionBankEditSaving) return;
  toggleMenu(false);
  setQuestionBankEditError('');
  fillQuestionBankEditDialog(question);
  openModalDialog('questionBankEditDialog', {focusTarget: $('questionBankEditPromptInput') || questionBankEditDialog()});
}

function closeQuestionBankEditDialog({restoreFocus = true, force = false} = {}) {
  if (state.questionBankEditSaving && !force) return;
  setQuestionBankEditError('');
  setQuestionBankEditSaving(false);
  closeModalDialog('questionBankEditDialog', {restoreFocus});
}

async function saveCurrentQuestionBankEdit() {
  const question = hydrateQuestionState(currentQuestion());
  if (!question?.questionBankId || state.questionLoading || state.questionSaving || state.markSaving || state.questionAnswerRefineLoading || state.questionBankEditSaving) return;
  let payload = null;
  try {
    payload = questionBankEditPayload(question);
  } catch (error) {
    setQuestionBankEditError(error.message || String(error));
    return;
  }
  if (!payload) return;
  state.questionBankEditSaving = true;
  setQuestionBankEditError('');
  setQuestionBankEditSaving(true);
  renderQuestionPanel();
  try {
    const res = await fetch(`/api/question-bank/${encodeURIComponent(question.questionBankId)}`, {
      method: 'PUT',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify(payload),
    });
    ...
    closeQuestionBankEditDialog({force: true});
    setMessage('문항 데이터를 저장했습니다.');
  } catch (error) {
    setQuestionBankEditError(error.message || String(error));
  } finally {
    state.questionBankEditSaving = false;
    if (questionBankEditDialog()?.hidden === false) setQuestionBankEditSaving(false);
    renderQuestionPanel();
  }
}
...
function syncUpdatedQuestionBankItem(item, {reloadOnFilterMismatch = true} = {}) {
  ...
  state.questions.forEach((question) => {
    ...
    current.card_id = String(item.card_id ?? current.card_id ?? '');
    if (item.question_type) {
      current.type = String(item.question_type || current.type || '');
      current.type_label = QUESTION_TYPE_LABELS[current.type] || current.type || current.type_label || '문제';
    }
    current.prompt = String(item.prompt ?? current.prompt ?? '');
    current.body = String(item.body ?? current.body ?? '');
    ...
    current.choices = Array.isArray(item.choices) ? item.choices : current.choices;
    current.answer_index = Number.isInteger(item.answer_index) ? item.answer_index : item.answer_index === null ? null : current.answer_index;
    current.topic = String(item.topic ?? current.topic ?? '');
    current.fieldName = String(item.field_name ?? current.fieldName ?? '');
    current.category = String(item.category ?? current.category ?? '');
    current.keywords = Array.isArray(item.keywords) ? item.keywords : current.keywords;
    current.difficulty = String(item.difficulty ?? current.difficulty ?? '');
    current.issuer = String(item.issuer ?? current.issuer ?? '');
    current.sourceLocation = String(item.source_location ?? current.sourceLocation ?? '');
    current.section = String(item.section ?? current.section ?? '');
    current.points = Number.isInteger(item.points) ? item.points : item.points === null ? null : current.points;
    current.expectedTimeSeconds = Number.isInteger(item.expected_time_seconds) ? item.expected_time_seconds : item.expected_time_seconds === null ? null : current.expectedTimeSeconds;
    current.answerGuide = String(item.answer_guide ?? current.answerGuide ?? '');
    current.sessionMode = normalizeQuestionSessionMode(item.session_mode ?? current.sessionMode ?? 'practice');
...
function renderQuestionBankBrowser() {
  ...
  list.innerHTML = state.questionBankItems.map((item, index) => {
    ...
    const topic = renderQuestionKeywordLinks(item.keywords, {interactive: true, question: item})
      || escapeHtml([
...
function hydrateQuestionState(question) {
  ...
  if (typeof question.subquestionMode !== 'boolean') question.subquestionMode = false;
  if (!Array.isArray(question.subquestionAnswers)) question.subquestionAnswers = [];
  const detectedSubquestionLabels = questionSubquestionLabels(question);
  if (detectedSubquestionLabels.length >= 2 && question.subquestionModeOverride !== 'off') {
    question.subquestionMode = true;
...
function questionAnswerSaveButtonHtml(questionSaveBusy, subquestionIndex = null) {
  const indexAttribute = Number.isInteger(subquestionIndex) ? ` data-question-subquestion-index="${subquestionIndex}"` : '';
  return `<div class="question-answer-save-actions"><button class="question-toolbar-button question-answer-save-button" type="button" data-question-answer-save="1"${indexAttribute} ${questionSaveBusy ? 'disabled' : ''}>답안 저장</button></div>`;
}

function questionAnswerGuideHtml(answerGuide) {
  if (!answerGuide) return '';
  return `<div class="question-answer-guide-wrap"><button class="question-info-button" type="button" data-question-info-toggle="1" aria-expanded="false" title="답안 가이드 보기" aria-label="답안 가이드 보기">i</button><div class="question-answer-guide question-markdown" hidden><strong>답안 가이드</strong>${renderQuestionMarkdown(answerGuide)}</div></div>`;
}
...
function questionCodeEditorHtml(question, questionSaveBusy, answerGuideHtml = '', subquestionIndex = null, showControls = true) {
  const layoutMode = questionLayoutMode();
  const layoutButtonLabel = layoutMode === 'horizontal' ? '세로 보기' : '가로 보기';
  const isSubquestion = Number.isInteger(subquestionIndex);
  const language = isSubquestion ? questionSubquestionFormat(question, subquestionIndex) : questionCodeLanguage(question) || 'text';
  const answers = isSubquestion ? ensureQuestionSubquestionAnswers(question) : [];
  const label = isSubquestion ? `소문제 ${question.subquestionLabels[subquestionIndex]}` : '코드 답안';
  const textareaId = isSubquestion && subquestionIndex > 0 ? `questionSubanswerInput-${subquestionIndex + 1}` : 'questionAnswerInput';
  const value = isSubquestion ? answers[subquestionIndex] || '' : question.userAnswer || '';
  const subquestionToggle = !question.subquestionMode ? `<button class="question-toolbar-button" type="button" data-subquestion-toggle="on" ${questionSaveBusy ? 'disabled' : ''}>소문제별 답안</button>` : '';
  const formatControl = isSubquestion
    ? `<label class="question-code-language-label" for="questionSubquestionFormat-${subquestionIndex}">형식</label><select id="questionSubquestionFormat-${subquestionIndex}" class="question-code-language" data-question-subquestion-format="${subquestionIndex}" aria-label="소문제 ${question.subquestionLabels[subquestionIndex]} 답안 형식">${questionSubquestionFormatOptions(language)}</select>`
    : `<label class="question-code-language-label" for="questionCodeLanguage">언어</label><select id="questionCodeLanguage" class="question-code-language" data-question-code-language aria-label="코드 언어">${questionCodeLanguageOptions(language)}</select>`;
  const manageControls = isSubquestion ? `${showControls ? `<button class="question-toolbar-button" type="button" data-subquestion-add ${questionSaveBusy ? 'disabled' : ''}>소문제 추가</button>` : ''}<button class="question-subquestion-remove" type="button" data-subquestion-remove="${subquestionIndex}" aria-label="소문제 ${question.subquestionLabels[subquestionIndex]} 삭제" ${questionSaveBusy ? 'disabled' : ''}>삭제</button>` : '';
  const sharedControls = showControls ? `<button class="question-toolbar-button" type="button" data-question-layout-toggle aria-pressed="${layoutMode === 'horizontal' ? 'true' : 'false'}">${layoutButtonLabel}</button>${subquestionToggle}<button class="question-toolbar-button" type="button" data-question-code-editor-toggle="off" ${questionSaveBusy ? 'disabled' : ''}>일반 답안</button>` : '';
...
function initializeQuestionCodeEditor() {
  const textareas = [...document.querySelectorAll('.question-code-editor-input')];
  if (!textareas.length || typeof window.CodeMirror !== 'function') return;
  questionCodeMirrorInstances = textareas.map((textarea) => {
    const language = textarea.dataset.codeLanguage || 'text';
    const editor = window.CodeMirror.fromTextArea(textarea, {
      ...
      extraKeys: {
        'Ctrl-Space': (instance) => instance.showHint({hint: questionCodeHint, completeSingle: false}),
        'Cmd-Space': (instance) => instance.showHint({hint: questionCodeHint, completeSingle: false}),
      },
    });
    editor.on('inputRead', (instance, change) => {
      if (change.text?.some((part) => /[A-Za-z_$]/.test(part))) instance.showHint({hint: questionCodeHint, completeSingle: false});
    });
...
function questionSubquestionDraftHtml(question, questionSaveBusy, answerGuideHtml = '') {
  ...
  const labels = question.subquestionLabels;
  const layoutMode = questionLayoutMode();
  const layoutToggleHtml = `<button class="question-toolbar-button" type="button" data-question-layout-toggle aria-pressed="${layoutMode === 'horizontal' ? 'true' : 'false'}">${layoutMode === 'horizontal' ? '세로 보기' : '가로 보기'}</button>`;
  const fieldsHtml = answers.map((answer, index) => {
    const format = questionSubquestionFormat(question, index);
...
function questionSubquestionTextFieldHtml(question, questionSaveBusy, index, answerGuideHtml = '') {
  ...
  const format = questionSubquestionFormat(question, index);
  const answerId = index === 0 ? 'questionAnswerInput' : `questionSubanswerInput-${index + 1}`;
  const manageControls = `${index === 0 ? `<button class="question-toolbar-button" type="button" data-subquestion-add ${questionSaveBusy ? 'disabled' : ''}>소문제 추가</button>` : ''}<button class="question-subquestion-remove" type="button" data-subquestion-remove="${index}" aria-label="소문제 ${labels[index]} 삭제" ${questionSaveBusy ? 'disabled' : ''}>삭제</button>`;
  return `<div class="question-subquestion-field question-subquestion-text-field question-surface"><div class="question-subquestion-item-head"><label class="question-answer-label" for="${answerId}">소문제 ${escapeHtml(labels[index])}</label><div class="question-subquestion-format"><label class="question-code-language-label" for="questionSubquestionFormat-${index}">형식</label><select id="questionSubquestionFormat-${index}" class="question-code-language" data-question-subquestion-format="${index}" aria-label="소문제 ${escapeHtml(labels[index])} 답안 형식">${questionSubquestionFormatOptions(format)}</select>${manageControls}</div></div><textarea id="${answerId}" class="question-answer-input question-subanswer-input" data-question-subquestion-index="${index}" rows="${question.type === 'essay' ? 6 : 4}" placeholder="소문제 ${escapeHtml(labels[index])} 답안을 작성하세요." ${questionSaveBusy ? 'disabled' : ''}>${escapeHtml(answers[index] || '')}</textarea>${questionAnswerSaveButtonHtml(questionSaveBusy, index)}${answerGuideHtml}</div>`;
}
...
async function refineCurrentQuestionAnswer() {
  const question = hydrateQuestionState(currentQuestion());
  if (!question || !question.questionBankId || state.questionLoading || state.questionSaving || state.markSaving || state.questionAnswerRefineLoading || state.questionBankEditSaving) return;
...
function renderQuestionPanel() {
  ...
  setQuestionControlsDisabled(state.questionLoading || state.questionSaving || state.markSaving || state.questionAnswerRefineLoading || state.questionBankEditSaving);
...
return `<li><button class="question-choice${isAnswer ? ' answer' : ''}${isSelected ? ' selected' : ''}${isWrongSelection ? ' wrong selected-wrong' : ''}" type="button" data-choice-index="${index}" aria-pressed="${isSelected ? 'true' : 'false'}" ${questionSaveBusy ? 'disabled' : ''}><span class="question-choice-body">${renderMarkdownInline(choice)}</span>${badges.length ? `<span class="question-choice-badges">${badges.join('')}</span>` : ''}</button></li>`;
  ...
  const linkedKeywordCard = linkedQuestionKeywordCard(question) || reviewCard;
  const questionKeywordHtml = renderQuestionKeywordLinks(question.keywords, {
    interactive: true,
    question,
    reviewCard: linkedKeywordCard,
  });
  ...
  const subquestionAnswers = questionNeedsManualGrading(question) && question.subquestionMode ? ensureQuestionSubquestionAnswers(question) : [];
  const hasSubquestionAnswers = subquestionAnswers.length > 0;
  const codeEditorHtml = questionCodeEditorEnabled
    ? (hasSubquestionAnswers
      ? `<div class="question-subquestion-code-list">${subquestionAnswers.map((_answer, index) => questionSubquestionFormat(question, index) === 'text' ? questionSubquestionTextFieldHtml(question, questionSaveBusy, index, index === subquestionAnswers.length - 1 ? answerGuideHtml : '') : questionCodeEditorHtml(question, questionSaveBusy, index === subquestionAnswers.length - 1 ? answerGuideHtml : '', index, index === 0)).join('')}</div>`
      : questionCodeEditorHtml(question, questionSaveBusy, answerGuideHtml))
    : '';
  const codeEditorToggleHtml = questionNeedsManualGrading(question) && !questionCodeEditorEnabled
    ...
    : '';
  const subquestionToggleHtml = questionNeedsManualGrading(question) && !hasSubquestionAnswers
    ? `<button class="question-toolbar-button" type="button" data-subquestion-toggle="on" ${questionSaveBusy ? 'disabled' : ''}>소문제별 답안</button>`
    : '';
  const answerDraftHtml = questionCodeEditorEnabled ? '' : hasSubquestionAnswers ? questionSubquestionDraftHtml(question, questionSaveBusy, answerGuideHtml) : questionNeedsManualGrading(question) ? `
    <div class="question-answer-draft question-surface">
      <div class="question-answer-draft-head"><label class="question-answer-label" for="questionAnswerInput">내 답안</label><div class="question-code-editor-actions">${layoutToggleHtml}${subquestionToggleHtml}${codeEditorToggleHtml}</div></div>
      ...
      ${questionAnswerSaveButtonHtml(questionSaveBusy)}
      ${answerGuideHtml}
  ...
  const questionInfoFields = [
    {label: '키워드', html: questionKeywordHtml},
  ...
  const questionInfoHtml = (questionInfoFields.length || linkedKeywordCard) ? `
    <div class="question-surface question-info-box">
      <strong class="question-answer-meta-title">문항 정보</strong>
      <div class="question-answer-meta-grid">
        ${questionInfoFields.map((item) => `<div class="question-answer-meta-item"><span class="question-answer-meta-label">${escapeHtml(item.label)}</span><div class="question-answer-meta-value${item.label === '키워드' ? ' question-keyword-list' : ''}">${item.html || escapeHtml(item.value)}</div></div>`).join('')}
        ${linkedKeywordCard ? `<div class="question-answer-meta-item"><span class="question-answer-meta-label">원본 카드</span><button class="question-answer-meta-card-button" type="button" data-question-open-card="1">${escapeHtml(linkedKeywordCard.term || linkedKeywordCard.id || '원본 카드로 이동')}</button></div>` : ''}
      </div>
    </div>` : '';
  const sessionSummary = questionSessionScoreSummary();
  ...
  const embedTopbarHtml = questionBankEmbedMode() ? `
    <div class="question-embed-topbar question-surface${question.judgment === 'wrong' ? ' is-wrong' : ''}${state.questionSessionFinishedAt ? ' is-finished' : ''}">
      ...
      <div class="question-embed-topbar-actions">
        <button class="question-toolbar-button" type="button" data-question-nav="prev" ${state.questionLoading || questionBusy || state.questionIndex <= 0 ? 'disabled' : ''}>이전</button>
...
destroyQuestionCodeEditor();
  ...
  $('revealAnswerBtn').disabled = state.questionLoading || questionBusy || !question;
  if ($('revealAnswerBtn')) {
    $('revealAnswerBtn').textContent = revealLocked ? '정답 잠금' : '정답';
    $('revealAnswerBtn').title = revealLocked ? '정답잠금 버튼을 눌러 제출하고 정답을 확인하세요.' : '정답/해설 보기';
  }
  $('openQuestionCardBtn').disabled = state.questionLoading || questionBusy || !reviewCard;
...
function goToQuestionKeyword(keyword, cardId = '') {
  const text = String(keyword || '').trim();
  if (!text) return false;
  const normalizedCardId = String(cardId || '').trim();
  const card = normalizedCardId
    ? state.cards.find((item) => String(item?.id || '').trim() === normalizedCardId) || findCardByKeyword(text)
    : findCardByKeyword(text);
  if (!card) {
...
$('questionBankList')?.addEventListener('click', (event) => {
  ...
  if (keywordButton) {
    event.preventDefault();
    event.stopPropagation();
    goToQuestionKeyword(keywordButton.dataset.questionKeyword || '', keywordButton.dataset.questionCardId || '');
    return;
  }
...
$('questionCard')?.addEventListener('click', (event) => {
  ...
  if (infoToggle) {
    const guide = infoToggle.closest('.question-answer-guide-wrap')?.querySelector('.question-answer-guide');
    if (!guide) return;
  ...
  if (answerSaveButton) {
    saveQuestionAttempt(currentQuestion()).catch(() => {});
    return;
  }
  const subquestionToggle = event.target.closest('[data-subquestion-toggle]');
  if (subquestionToggle) {
    const question = hydrateQuestionState(currentQuestion());
    if (!question) return;
    const enabled = subquestionToggle.dataset.subquestionToggle === 'on';
    question.subquestionModeOverride = enabled ? 'on' : 'off';
    if (enabled) {
      question.subquestionMode = true;
      const labels = questionSubquestionLabels(question);
      question.subquestionLabels = labels.length >= 2 ? labels : ['1', '2'];
      ensureQuestionSubquestionAnswers(question);
    } else {
      const answers = ensureQuestionSubquestionAnswers(question);
      question.subquestionMode = false;
      question.userAnswer = answers.join('\n\n');
    }
    renderQuestionPanel();
    window.setTimeout(() => (questionCodeMirrorInstances[0] || $('questionAnswerInput'))?.focus(), 0);
    return;
  }
  const addSubquestion = event.target.closest('[data-subquestion-add]');
  if (addSubquestion) {
    const question = hydrateQuestionState(currentQuestion());
    if (!question) return;
    const answers = ensureQuestionSubquestionAnswers(question);
    question.subquestionLabels.push(String(question.subquestionLabels.length + 1));
    question.subquestionAnswers = [...answers, ''];
    ensureQuestionSubquestionAnswers(question);
    renderQuestionPanel();
    window.setTimeout(() => document.querySelector(`#questionSubanswerInput-${question.subquestionLabels.length}`)?.focus(), 0);
    return;
  }
  const removeSubquestion = event.target.closest('[data-subquestion-remove]');
  if (removeSubquestion) {
    const question = hydrateQuestionState(currentQuestion());
    if (!question) return;
    const index = Number.parseInt(removeSubquestion.dataset.subquestionRemove || '', 10);
    const answers = ensureQuestionSubquestionAnswers(question);
    if (!Number.isInteger(index)) return;
    if (answers.length <= 1) {
      question.subquestionMode = false;
      question.subquestionModeOverride = 'off';
      question.subquestionLabels = [];
      question.subquestionFormats = [];
      question.subquestionAnswers = [];
      question.userAnswer = '';
      renderQuestionPanel();
      return;
    }
    question.subquestionLabels.splice(index, 1);
    question.subquestionFormats.splice(index, 1);
    question.subquestionAnswers = answers.filter((_answer, answerIndex) => answerIndex !== index);
    ensureQuestionSubquestionAnswers(question);
    renderQuestionPanel();
    return;
  }
  ...
  if (layoutToggle) {
    ...
    renderQuestionPanel();
    if (nextMode) window.setTimeout(() => questionCodeMirrorInstances[0]?.focus(), 0);
    return;
  }
  ...
  if (codeToggle) {
    ...
    renderQuestionPanel();
    if (question.codeEditorEnabled) window.setTimeout(() => questionCodeMirrorInstances[0]?.focus(), 0);
    return;
  }
  const keywordButton = event.target.closest('[data-question-keyword]');
  if (keywordButton) {
  ...
  if (event.target.closest('[data-question-edit="1"]')) {
    openQuestionBankEditDialog();
    return;
  }
...
$('questionCard')?.addEventListener('change', (event) => {
  const subquestionSelect = event.target.closest('[data-question-subquestion-format]');
  if (subquestionSelect) {
    const question = hydrateQuestionState(currentQuestion());
    if (!question) return;
    const index = Number.parseInt(subquestionSelect.dataset.questionSubquestionFormat || '', 10);
    if (!Number.isInteger(index)) return;
    ensureQuestionSubquestionAnswers(question);
    question.subquestionFormats[index] = normalizeQuestionCodeLanguage(subquestionSelect.value) || 'text';
...
$('questionBankEditCloseBtn')?.addEventListener('click', () => closeQuestionBankEditDialog());
$('questionBankEditCancelBtn')?.addEventListener('click', () => closeQuestionBankEditDialog());
$('questionBankEditSaveBtn')?.addEventListener('click', saveCurrentQuestionBankEdit);
...
$('questionBankEditDialog')?.addEventListener('click', (event) => {
  if (event.target === $('questionBankEditDialog')) closeQuestionBankEditDialog();
});
...
function applyQuestionBankEmbedShell() {
  if (!questionBankEmbedMode()) return;
  document.body.classList.add('question-bank-embed');
  document.body.style.background = '#fff';
  document.querySelectorAll('.topbar, #controlsPanel, .progress-line, #filterRow, .flashcard, .mark-row, #closeQuestionModeBtn, #questionBankToggleBtn, #questionBankBrowser').forEach((element) => {
    element.hidden = true;
  });
}

applyQuestionBankEmbedShell();
