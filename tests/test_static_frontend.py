...
class StaticFrontendTests(unittest.TestCase):
    ...
    def test_calendar_page_and_menu_links_are_present(self):
        ...
        self.assertIn('id="summaryDeadlineBtn"', CALENDAR_HTML)
        self.assertIn('id="summaryOpenBtn"', CALENDAR_HTML)
        self.assertIn('id="summaryNextBtn"', CALENDAR_HTML)
        self.assertIn('id="summaryFilterBtn"', CALENDAR_HTML)
        self.assertIn('id="mainTabCalendarBtn"', CALENDAR_HTML)
        self.assertIn('id="mainTabListBtn"', CALENDAR_HTML)
        self.assertIn('id="mainTabFiltersBtn"', CALENDAR_HTML)
        self.assertIn('id="mainTabInstitutionsBtn"', CALENDAR_HTML)
        self.assertIn('data-main-panel="calendar"', CALENDAR_HTML)
        self.assertIn('data-main-panel="filters"', CALENDAR_HTML)
        self.assertIn('id="activeFilterTags"', CALENDAR_HTML)
        self.assertIn('id="filterResultCount"', CALENDAR_HTML)
        self.assertIn('id="eventListModeSelectedBtn"', CALENDAR_HTML)
        self.assertIn('role="dialog"', CALENDAR_HTML)
        self.assertIn('aria-modal="true"', CALENDAR_HTML)
        self.assertIn('aria-pressed="false"', CALENDAR_HTML)
        self.assertIn('id="selectedEventPeek"', CALENDAR_HTML)
        self.assertIn('id="selectedEventOpenBtn"', CALENDAR_HTML)
        self.assertIn('id="calendarDetailDrawer"', CALENDAR_HTML)
        self.assertIn('id="overviewHeadline"', CALENDAR_HTML)
        ...
        self.assertIn('function renderFilterOptions(containerId', CALENDAR_JS)
        self.assertIn('function resetFilterGroup(group)', CALENDAR_JS)
        self.assertIn('function renderSummaryBar(events = filteredEvents())', CALENDAR_JS)
        self.assertIn('function renderSelectedEventPeek(event)', CALENDAR_JS)
        self.assertIn('function openDetailDrawer()', CALENDAR_JS)
        self.assertIn("button.setAttribute('tabindex', active ? '0' : '-1');", CALENDAR_JS)
        self.assertIn("setAttribute('aria-pressed', String(selectedMode))", CALENDAR_JS)
        self.assertIn("button.addEventListener('keydown', handleMainTabKeydown);", CALENDAR_JS)
        self.assertIn("event.key !== 'Tab' || !calendarState.detailOpen", CALENDAR_JS)
        self.assertIn("calendarState.lastFocusedElement = activeElement && typeof activeElement.focus === 'function' ? activeElement : null;", CALENDAR_JS)
        self.assertIn('function applyCalendarEventTone(info)', CALENDAR_JS)
        self.assertIn('Google Calendar에 추가', CALENDAR_JS)
        self.assertIn("document.querySelectorAll('[data-main-tab]')", CALENDAR_JS)
        self.assertIn("$('summaryFilterBtn')?.addEventListener('click'", CALENDAR_JS)
        self.assertIn("bindFilterGroup('institutionFilters'", CALENDAR_JS)
        self.assertIn('.calendar-main-tabs', CALENDAR_CSS)
        self.assertIn('.summary-card', CALENDAR_CSS)
    ...
    def test_wiki_ui_and_flashcard_links_are_present(self):
        ...
        self.assertIn('id="wikiBatchAiToggleBtn"', WIKI_HTML)
        self.assertIn('id="wikiBatchAiPanel"', WIKI_HTML)
        self.assertIn('id="wikiBatchAiFormat"', WIKI_HTML)
        self.assertIn('id="wikiBatchCurrentBtn"', WIKI_HTML)
        self.assertIn('id="wikiBatchSelectedBtn"', WIKI_HTML)
        self.assertIn('id="wikiBatchAiHint"', WIKI_HTML)
        ...
        self.assertIn('WIKI_BATCH_AI_MODE_KEY', WIKI_JS)
        self.assertIn('function applyWikiSidebarState(', WIKI_JS)
        self.assertIn('function applyWikiBatchAiMode(', WIKI_JS)
        self.assertIn('function toggleWikiBatchAiMode(', WIKI_JS)
        ...
        self.assertIn("toggleBtn.textContent = wikiState.batchAiMode ? '목차' : 'AI'", WIKI_JS)
        ...
        self.assertIn('function wikiEnhanceInlineImages(page = wikiState.page)', WIKI_JS)
        self.assertIn('function wikiRegenerateInlineImage(imageIndex)', WIKI_JS)
        self.assertIn('/api/wiki/ai-jobs', WIKI_JS)
        self.assertIn('/api/wiki/ai-jobs/${encodeURIComponent(jobId)}', WIKI_JS)
        self.assertIn('wiki-inline-image-format', WIKI_JS)
        self.assertIn('WIKI_IMAGE_PROMPT_STORAGE_KEY', WIKI_JS)
        self.assertIn('function wikiToggleImagePromptEditor(imageIndex)', WIKI_JS)
        self.assertIn('function wikiToggleSectionPromptEditor(sectionIndex)', WIKI_JS)
        self.assertIn('function wikiEnhanceSectionHeadings(page = wikiState.page)', WIKI_JS)
        self.assertIn('function wikiGenerateSectionImage(sectionIndex)', WIKI_JS)
        self.assertIn('function wikiBatchSectionTargets(page = wikiState.page)', WIKI_JS)
        self.assertIn('function wikiIsGeneratedSectionImage(image = {})', WIKI_JS)
        self.assertIn('function wikiPageAiJobSpecs(page = wikiState.page, format = \'png\'', WIKI_JS)
        self.assertIn('function wikiQueueCurrentPageAi()', WIKI_JS)
        self.assertIn('function wikiQueueSelectedDocsAi()', WIKI_JS)
        self.assertIn('function wikiQueueAiJobs(jobSpecs, label)', WIKI_JS)
        self.assertIn('function wikiPollAiJobs()', WIKI_JS)
        self.assertIn('prompt_template: promptOverride', WIKI_JS)
        self.assertIn('wiki-inline-image-prompt-textarea', WIKI_JS)
        self.assertIn('sectionAiLoadingIndex', WIKI_JS)
        self.assertIn('data-wiki-ai-source', WIKI_JS)
        self.assertIn('wikiBatchCurrentBtn', WIKI_JS)
        self.assertIn('wikiBatchSelectedBtn', WIKI_JS)
        self.assertIn('wikiBatchAiToggleBtn', WIKI_JS)
        self.assertIn('백그라운드에서 처리합니다.', WIKI_JS)
        self.assertIn('function wikiTrackAiJob(job, label)', WIKI_JS)
        ...
        self.assertIn('.wiki-inline-image-shell', STYLE_CSS)
        self.assertIn('.wiki-inline-image-ai', STYLE_CSS)
        self.assertIn('.wiki-inline-image-format', STYLE_CSS)
        self.assertIn('.wiki-inline-image-prompt-editor', STYLE_CSS)
        self.assertIn('.wiki-inline-image-edit-btn', STYLE_CSS)
        self.assertIn('.wiki-batch-ai-bar', STYLE_CSS)
        self.assertIn('.wiki-toc-ai-checkbox', STYLE_CSS)
        self.assertIn('.wiki-section-ai', STYLE_CSS)
        self.assertIn('.wiki-section-prompt-editor', STYLE_CSS)
