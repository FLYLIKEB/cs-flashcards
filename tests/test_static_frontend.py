...
class StaticFrontendSmokeTests(unittest.TestCase):
    ...
    def test_question_bank_page_smoke_has_filter_table_and_practice_shell(self):
        ...
        self.assertIn('bankPageLaunchSingleBtn', question_bank_html())
        self.assertIn("launch(selectedIndex(bankState.practiceStartIndex), {single: true})", question_bank_js())
    ...
    def test_question_bank_js_smoke_keeps_load_launch_and_persistence_hooks(self):
        ...
        self.assertIn('function launch(startIndex = 0, {reveal = true, single = false} = {})', question_bank_js())
    ...
    def test_question_answer_meta_is_minimal(self):
        ...
        self.assertIn("{label: '분야', value: question.fieldName || ''}", app_block)
        self.assertIn("{label: '섹션', value: question.section || ''}", app_block)
        self.assertIn("{label: '난이도', value: question.difficulty || ''}", app_block)
        self.assertNotIn("{label: '문제유형', value: question.topic || ''}", app_block)
        self.assertNotIn("{label: '기관', value: question.issuer || ''}", app_block)
        self.assertNotIn("{label: '출처', value: question.sourceLocation || ''}", app_block)
        css = style_css()
        self.assertIn('.question-answer-meta-title', css)
        self.assertIn('grid-template-columns: repeat(auto-fit, minmax(7rem, 1fr));', css)
        self.assertIn('font-size: .66rem;', css)
