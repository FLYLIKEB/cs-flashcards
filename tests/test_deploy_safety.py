...
class DeploySafetyTests(unittest.TestCase):
    ...
    def test_deploy_script_removes_whole_db_replacement_path(self):
    ...
    def test_sync_script_requires_explicit_columns_and_fixed_remote_db_path(self):
        self.assertNotIn('[--remote-db PATH]', SYNC_SCRIPT)
        self.assertIn('--columns 는 필수입니다. 원격 row 전체 덮어쓰기는 금지됩니다.', SYNC_SCRIPT)
